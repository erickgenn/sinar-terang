...
class AuthController extends BaseController
{
    ...
    {
        return view('auth/forgot_password_customer');
    }
    ...
    public function registerCustomerAuthEmail()
    {
        ...
        $customerModel = new CustomerModel();
        $search_email = $customerModel->where('email', $email_input)->first();
        ...
        $encrypted_token = AuthController::aes128Encrypt($data);

        $url_register = base_url('register/customer/auth') . "/" . $encrypted_token;
    ...
    public function registerCustomerAuth($token)
    {
        $session = session();

        $data = AuthController::aes128Decrypt($token);
    ...
    public function customerForgotAuth()
    {
        $session = session();
        $data = $this->request->getPost();
        $email = $data['email'];
        ...
        if (!isset($user)) {
            ...
            return redirect()->to('forgot_password/customer/index');
        } else {
            $encrypted_token = md5($user['email']);
            $url_changepass = base_url('forgot_password/customer/change_pass') . "/" . $encrypted_token;
            $email = \Config\Services::email();
            $email->setFrom('sinarterang.adm@gmail.com', "Admin Sinar Terang");
    ...
    public function customerForgotChange($token)
    {
        return view('auth/change_password', compact('token'));
    }

    public function customerForgotNew($token)
    {
        $session = session();
        $data = $this->request->getPost();
        ...
        if ($data['password'] != $data['confirm_password']) {
            $session->setFlashdata('password_different', "failed");
            return redirect()->to('forgot_password/customer/change_pass/' . $token);
        }
        ...
        if ($token == $encrypted_email) {
            ...
            $customerModel = new CustomerModel();
            $user = $customerModel->where('email', $data['email'])->first();

            $customerModel->update($user['id'], $data_update);

            $session->setFlashdata('password_changed', 'Email is incorrect!');
            return redirect()->to('login/customer');
        } else {
